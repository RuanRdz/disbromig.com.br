...
$public_sql = mysql_query("SELECT nome,senha FROM public WHERE id='1'");
while ($p=mysql_fetch_array($public_sql)) {
	$p_login = $p["nome"];
	$p_senha = $p["senha"];
}
...
// upload da planilha excel
	...
	# Atualiza no SQL
	$atualiza_public_sql = mysql_query("UPDATE public SET nome='".$n_login."', senha='".$n_senha."' WHERE id='1'") or die (mysql_error());
	
	# Atualiza Apache
	// remove atual
	htpasswd_remove($p_login);
	// salva novo
	htpasswd_salva($n_login, $n_senha);
...
/* MANIPULA .htpasswd
 ...
 Used : PHP and Smarty
*/
function htpasswd_salva($usr, $pwd) {
	$filename = '../estoque/.htpasswd';
	$data = $usr.":".htpasswd($pwd)."\n";
	if (is_writable($filename)) {
		...
		if (fwrite($handle, $data) === FALSE) {
			echo "Nao foi possivel salvar dados no arquivo ($filename)";
			exit;
		}
		...
		echo "O arquivo $filename nao pode ser alterado"; // not writable
	}
}
// function to show all users and passwords ( encrypted )
function showUser()
{      
     $file = file('../estoque/.htpasswd');
     $array = array();
     $count = count($file);
     for ($i = 0; $i < $count; $i++)
     {
...
function htpasswd_remove($usr) {
   $fileName = file('../estoque/.htpasswd');
   $pattern = "/". $username."/";
   foreach ($fileName as $key => $value) {
		if(preg_match($pattern, $value)) { $line = $key;  }
   }
   unset($fileName[$line]);
   if (!$fp = fopen('../estoque/.htpasswd', 'w+')) {
