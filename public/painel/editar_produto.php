...
if (isset($do) && $do == "alterar") {
	...
	$n_foto = $_FILES["foto"];
	$n_promo = addslashes($_REQUEST["promo"]);
	$n_promoValor = addslashes($_REQUEST["promo-valor"]);
	$n_status = addslashes($_REQUEST["status"]);
	$n_listagem = $_REQUEST["listagem"];
	$remover_foto = addslashes($_REQUEST["remover_foto"]);
	...
	if (isset($n_foto) && $n_foto["error"] != 4) {
			...
			else { die("A imagem forncedia n�o � v�lida, seu formato deve ser JPG, GIF ou PNG.<br><a href='javascript:history.go(-1)'>Clique aqui para voltar</a>"); }
